                    ...
                    &size) == 1))
    {
     ...
     // ACE_Filecache_Handle afh (ACE_TEXT_CHAR_TO_TCHAR (this->filename_),
      ...
      //this->peer ().recv_n (afh.address (), this->response_size_);
        char *a;
        a=new char[this->response_size_+1];
        memset(a,0,this->response_size_+1);
        this->peer ().recv_n (a, this->response_size_);
...
int
HTTP_Connector::connect (const char * url,string &outFilename)
{
  char host[BUFSIZ];
  u_short port;
  char path[BUFSIZ];
  ...
  int a= this->connector_.connect (hhptr, ACE_INET_Addr (port, host));

 outFilename=hhptr->in;
